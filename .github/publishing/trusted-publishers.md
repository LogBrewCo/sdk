...
Real npm runs require an explicit `npm_packages` set. Before publishing any
...
the shared publisher identity above, and only then include the package in a
real dispatch. Missing pages fail before the first immutable publish.
