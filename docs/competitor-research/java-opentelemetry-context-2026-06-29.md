...
- 2026-07-03 release-readiness follow-up: `.github/workflows/release-readiness.yml` now runs `bash scripts/real_user_java_opentelemetry_smoke.sh` after the core Java real-user smoke and before Java framework smokes, guarded by `tests/test_java_opentelemetry_smoke.py`. The routine push workflow remains leaner so expensive rich-trace package proof does not duplicate every local/pre-commit check.
