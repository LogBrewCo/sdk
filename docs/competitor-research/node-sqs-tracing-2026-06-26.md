...
The package intentionally avoids hidden/global AWS SDK patching, queue URL capture, queue ARN/account/region capture, arbitrary message attributes, message bodies, receipt handles, message IDs, MD5 values, payload sizes, baggage, tracestate, data-stream monitoring, stack traces, exception messages, support-ticket calls, hosted release-artifact claims, and EventBridge producer JSON mutation. The opt-in body parsing path returns only a normalized `traceparent` and never sends body, SNS message, EventBridge detail, or malformed propagation bytes to LogBrew.
...
Sentry, OpenTelemetry, and Datadog remain stronger for zero-code/global AWS SDK patching, SNS/EventBridge producer helpers, EventBridge JSON detail injection, data-stream monitoring, custom hooks, and automatic cloud resource metadata. LogBrew now has an opt-in one-client automatic SQS path plus explicit receive-side SNS/EventBridge envelope recovery, but should not add global AWS SDK patching or EventBridge producer mutation unless source-backed proof shows it can stay privacy-bounded, predictable, and stable enough to justify the coupling.
