...
## XHR Response Size Follow-Up - 2026-06-29

- Refreshed upstream heads before implementation: Datadog React Native remained `DataDog/dd-sdk-reactnative@92462dccefd689815d87dabbad0d41572cd06cca`, Sentry JavaScript was `getsentry/sentry-javascript@54e995da76381f18f61f39b0ceecadf5a0b06b11`, and Sentry React Native was `getsentry/sentry-react-native@578964579873bd2fa16107fb17d4621fcfee2b30`.
- Re-read Datadog React Native `packages/core/src/rum/instrumentation/resourceTracking/requestProxy/XHRProxy/XHRProxy.ts`: `reportXhr(...)` calculates response size, carries status, response-start timing, trace IDs, and optional GraphQL attributes into `ResourceReporter.reportResource(...)`.
- Re-read Datadog React Native `packages/core/src/rum/instrumentation/resourceTracking/requestProxy/XHRProxy/responseSize.ts`: `calculateResponseSize(...)` prefers `Content-Length` but can fall back to response body/string/blob/arraybuffer/json sizing.
- Re-read Datadog React Native `packages/core/src/rum/instrumentation/resourceTracking/requestProxy/XHRProxy/DatadogRumResource/ResourceReporter.ts`: `reportResource(...)` passes response size and timing metadata into native RUM resource stop.
- Re-read Sentry JavaScript `packages/browser-utils/src/instrument/xhr.ts`: `instrumentXHR()` patches XHR, records status at ready-state completion, intercepts `setRequestHeader`, and stores request body metadata for instrumentation handlers.
- LogBrew now narrows the response-size gap by adding `responseSizeBytes` to opt-in global XHR resource spans only when `getResponseHeader("Content-Length")` returns a non-negative finite value. It still does not inspect `response`, `responseText`, `responseType`, request bodies, response bodies, arbitrary headers, GraphQL payloads, baggage, or tracestate.
- Verification: RED `npm test --prefix js/logbrew-react-native -- --test-name-pattern "global XHR"` failed on missing `responseSizeBytes`; GREEN passed after adding Content-Length-only sizing. `npm test --prefix js/logbrew-react-native` and `bash scripts/real_user_react_native_smoke.sh` passed, and the installed instrumentation-kit example now proves XHR response size from a packed artifact.

## Tradeoffs

- LogBrew intentionally did not copy Sentry native scope sync, navigation auto-instrumentation, Datadog default XHR/fetch patching, body-derived response sizing, GraphQL payload extraction, Babel interaction rewriting, multi-format propagation, baggage, tracestate, replay, payload capture, or native bridge state.
