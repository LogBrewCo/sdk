...
- Easier typed `ureq`, `reqwest`, and Tower adoption than manual spans: the helpers preserve the app's agent/request/service ownership and original HTTP result while automatically timing, injecting propagation, and queuing the span.
...
- Installed-artifact proof is explicit: packaged crate smokes validate the README, exported API, manual emitted span, `ureq` capture path, `reqwest` capture path, Tower outbound layer, outgoing `traceparent`, and absence of unsafe metadata leakage.
...
- LogBrew does not yet provide typed Hyper client wrappers; non-`ureq`/non-`reqwest`/non-Tower apps still call the manual helper and set the returned header themselves.
