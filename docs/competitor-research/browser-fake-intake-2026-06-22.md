...
- Lighter than Sentry/Datadog/PostHog: no hidden global browser batching layer, no background queue worker, no sendBeacon fallback, no rate-limit store, and no persisted offline queue. Keepalive delivery and queue pressure are explicit and bounded instead of best-effort browser-dependent.
- Still worse than Sentry/Datadog/PostHog for production browser delivery under unload/offline/rate-limit pressure. Future work should add source-backed rate-limit handling and lifecycle delivery only when the SDK owns a framework/browser integration layer explicitly.
