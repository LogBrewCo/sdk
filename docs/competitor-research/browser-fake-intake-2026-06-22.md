...
- Datadog files/functions: `packages/browser-core/src/transport/batch.ts` (`startBatch`, `flush`), `packages/browser-core/src/transport/httpRequest.ts` (`createHttpRequest`, `sendOnExit`, exit-time send-beacon helper, fallback to fetch), and the retry helper for queueing, retry scheduling, and bandwidth accounting.
...
- Datadog uses browser batching plus explicit retry queue state, bandwidth accounting, sendBeacon-on-exit fallback, fetch fallback, and queue-full notifications.
- PostHog routes browser requests through a configurable request layer with queueing, rate-limit checks, callbacks, retry queue delegation, and a fire-and-forget `sendBeacon` transport for unload.
...
- 2026-06-23 update: browser connectivity recovery now flushes queued in-memory events when the browser emits `online`, reports `details.reason` as `online`, and removes the listener on `uninstall()`. This follows the competitor pattern of retrying buffered work after transport disruption, but stays explicit and memory-only unless the app opts into persisted delivery.
- 2026-07-03 update: `createPersistentBrowserTransport()` and `installLogBrewBrowser({ persistOffline })` now add explicit persisted fetch-batch replay through app-provided/Web Storage. Stored records contain bounded JSON batch bodies and owner markers only, not browser keys, request headers, cookies, query strings, hash fragments, or raw app payloads. Installed browser smoke proves direct replay, reload/new-context online replay, TypeScript declarations, CommonJS export presence, and no browser-key storage.
...
- Lighter than Sentry/Datadog/PostHog: no hidden global browser batching layer, no background queue worker, no default `sendBeacon` path that would need a different client-key delivery contract, and no local usage/quota derivation. Keepalive delivery, persisted fetch-batch replay, queue pressure, lifecycle/connectivity reason callbacks, and rate-limit recovery signals are explicit and bounded instead of best-effort browser-dependent.
- Still worse than Sentry/Datadog/PostHog for production browser delivery under abrupt unload pressure, sendBeacon-style exit delivery, autonomous batch scheduling, and category-aware rate-limit stores. Future work should add lifecycle delivery upgrades only when the SDK owns an authorization-safe browser contract explicitly.
...
- Green: installed browser smoke passed with `persistOffline`, direct persisted transport replay, reload/new-context online replay, TypeScript declaration coverage, CommonJS export coverage, and no browser-key storage.
