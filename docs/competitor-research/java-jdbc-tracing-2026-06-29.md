...
- GREEN: `bash scripts/check_java_package.sh` passed with 32 core Java tests, 6 trace tests, 2 servlet tests, 2 span-event tests, 3 OpenTelemetry tests, 4 operation-tracing tests, 10 JDBC-tracing tests, 2 support-ticket tests, Maven metadata, javadocs, source jar, binary jar, README checks, and packaged examples.
