...
- Datadog Java tracer: `https://github.com/DataDog/dd-trace-java.git` at `dd95ecc5f440436eda34ff94169cec85900abadd` for the Spring Kafka follow-up, with earlier queue-agent notes from `04ea23af81f738f81dc0f75ecbd99e83f9ab1d6a`.
...
LogBrew Java now also adds `LogBrewSpringKafkaTracing.recordInterceptor(...)` for apps that already use Spring Kafka:

- Apps register the returned `RecordInterceptor` with their own listener container factory.
- The helper continues one incoming W3C `traceparent`, makes the child trace active while listener code runs, and emits one `spring.kafka.process:<topic>` span on `success(...)`, `failure(...)`, or Spring Kafka thread-state clear.
- It derives primitive non-negative `timeInQueueMs` from the Kafka record timestamp when available.
- It preserves app-owned delegate interceptors and treats malformed propagation as a non-fatal `onError(...)` diagnostic.
- It filters configured metadata through the dependency-span privacy blocklist and records only primitive framework/source/topic/status/duration/sampled/error-type data.
- It avoids producer wrapping, hidden auto-configuration, record keys, values, offsets, arbitrary headers, broker addresses, consumer group IDs, baggage, tracestate, exception messages, and stack traces.

## Honest Comparison

LogBrew is now better than PostHog Java for queue trace propagation and safer/lighter than Sentry, Datadog, and OpenTelemetry for explicit app-owned queue carriers with installed-artifact proof. The new Spring Kafka interceptor closes the most direct consumer-process ergonomics gap against Sentry for applications that prefer explicit registration and privacy-bounded payloads. LogBrew is still worse than Sentry for producer wrapping, automatic Spring Kafka post-processing, Sentry timing-header interop, and out-of-the-box Kafka setup depth; worse than OpenTelemetry for standard automatic Kafka/JMS instrumenters and batch span-link depth; and worse than Datadog for automatic broker breadth, data-stream metadata, agent-driven coverage, and automatic time-in-queue spans.

## Verification

- Focused package gate: `bash scripts/check_java_package.sh` passed with Java operation tracing tests plus Spring Kafka record-interceptor tests for incoming trace continuation, active trace scope during listener work, sanitized span capture, malformed propagation diagnostics, metadata redaction, and `clearThreadState(...)` completion.
- Spring Kafka installed-artifact gate: `bash scripts/real_user_java_spring_kafka_smoke.sh` passed from a packaged jar, registering the interceptor against real `ConsumerRecord`/`RecordHeaders`, validating W3C correlation, validating `timeInQueueMs`, and proving key/value/header/metadata redaction.
...
- Spring Boot gate: `bash scripts/real_user_spring_boot_smoke.sh` passed with `spring-boot@4.0.6`.
...
Next useful Java trace improvements are producer-side Kafka/Spring Kafka wrapping or post-processing, JMS helper/package coverage, batch receive/process convenience helpers, richer messaging semantic attributes, baggage/tracestate only if explicitly justified, and OpenTelemetry processor/exporter interop. Do not add automatic broker/client patching to core `logbrew-sdk`.
