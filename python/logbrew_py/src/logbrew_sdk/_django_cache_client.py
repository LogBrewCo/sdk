...
_INSTRUMENTATION_ATTR = "_logbrew_django_cache_instrumentation"
...
def instrument_django_cache_with_logbrew_spans(
    ...
    *,
    ...
    if not methods:
        raise TypeError("cache must expose at least one supported Django cache method")

    existing = _existing_instrumentation(cache)
    if existing is not None and existing.installed:
        return existing

    instrumentation = LogBrewDjangoCacheInstrumentation(
        cache=cache,
        methods=methods,
        client=client,
        ...
        metadata=_django_cache_metadata(metadata),
...
class LogBrewDjangoCacheInstrumentation:
    ...
    def install(self) -> None:
        """Wrap supported app-owned Django cache methods."""
    ...
    def uninstall(self) -> None:
        """Put original app-owned Django cache methods back."""
...
def _existing_instrumentation(cache: Any) -> LogBrewDjangoCacheInstrumentation | None:
    try:
        instrumentation = getattr(cache, _INSTRUMENTATION_ATTR, None)
    except Exception:
        return None
    if isinstance(instrumentation, LogBrewDjangoCacheInstrumentation):
        return instrumentation
    return None


def _remember_instrumentation(cache: Any, instrumentation: LogBrewDjangoCacheInstrumentation) -> None:
    try:
...
def _forget_instrumentation(cache: Any, instrumentation: LogBrewDjangoCacheInstrumentation) -> None:
    try:
...
def _apply_result_metadata(
    ...
    if method_name == "get":
        request.cache_hit = result != _django_get_default(args, kwargs)
...
def _django_get_default(args: tuple[Any, ...], kwargs: Mapping[str, Any]) -> Any:
    if len(args) >= 2:
        return args[1]
...
def _django_cache_metadata(metadata: Mapping[str, Any] | None) -> _instrumentation.Metadata:
    safe_metadata = _instrumentation.compact_metadata_without_keys(metadata, (*_CACHE_METADATA_DENYLIST, "connection"))
    safe_metadata["framework"] = "django-cache"
    return safe_metadata


def _default_event_id() -> str:
    return f"evt_python_django_cache_{uuid4().hex}"
