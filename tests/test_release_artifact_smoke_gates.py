...
class ReleaseArtifactSmokeGateTests(unittest.TestCase):
    ...
    def test_vite_smoke_links_runtime_browser_error_to_release_artifact_debug_id(self) -> None:
        ...
        self.assertIn('assert metadata["releaseArtifactDebugId"] == sdk_issue_symbolication["debugId"]', smoke)
        self.assertIn('assert metadata["source"] == "browser.error"', smoke)
        self.assertIn('assert metadata["path"] == "/checkout"', smoke)
        ...
        self.assertIn('assert "hidden@example.test" not in serialized_issue_event', smoke)
        self.assertIn('assert "cache=placeholder" not in serialized_issue_event', smoke)
        self.assertIn("assert tmp_dir not in serialized_issue_event", smoke)
    ...
    def test_next_smoke_links_runtime_browser_error_to_release_artifact_debug_id(self) -> None:
        ...
        self.assertIn("--source-root", smoke)
        self.assertIn('source_context = symbolication_report["sourceContext"]', smoke)
        self.assertIn('assert "next checkout exploded" in source_context["lines"][0]["text"]', smoke)
