...
assertTrue($singleTransportResponse->batches === 1, 'a direct transport response must represent one request batch');
...
assertTrue($countResponse->statusCode === 202, 'count split must return the final accepted status');
assertTrue($countResponse->attempts === 4, 'count split must aggregate all transport attempts');
assertTrue($countResponse->batches === 3, 'count split must report three accepted batches');
assertTrue(count($countTransport->sentBodies) === 4, 'count split must retry once and send three batches');
assertTrue($countTransport->sentBodies[0] === $countTransport->sentBodies[1], 'retry body must be byte-identical');
assertTrue(!str_contains($countTransport->sentBodies[0], "\n"), 'transport batches must use compact JSON');
assertTrue(
...
assertTrue($countClient->pendingEvents() === 0, 'count split must acknowledge every accepted event');
...
assertTrue($byteResponse->batches === 2, 'exact byte limit must split two events into two batches');
assertTrue(count($byteTransport->sentBodies) === 2, 'exact byte limit must send twice');
assertTrue(
...
assertTrue($batchDropClient->pendingEvents() === 0, 'an event larger than one batch must not enter the queue');
assertTrue($batchDropReason === 'event_too_large', 'batch-oversized events must use the stable reason');
...
assertTrue($partialClient->pendingEvents() === 3, 'partial success must remove only the accepted prefix');
assertTrue(
...
assertTrue($partialResponse->batches === 2, 'retained partial queue must drain in two batches');
assertTrue(
...
assertTrue($reentrantResponse->batches === 1, 'outer flush must accept its original snapshot');
assertTrue($reentrantClient->pendingEvents() === 1, 'capture during transport I/O must remain queued');
assertTrue($reentrantTransport->flushError?->codeName === 'flush_error', 'reentrant flush must fail with a stable code');
assertTrue(
...
assertTrue($shutdownTransport->captureError?->codeName === 'shutdown_error', 'shutdown must reject transport-time capture');
assertTrue($shutdownClient->pendingEvents() === 1, 'failed shutdown must retain the original event');
$shutdownClient->log('evt_after_failed_shutdown', '2026-07-12T10:00:08Z', ['message' => 'reopened', 'level' => 'info']);
assertTrue($shutdownClient->pendingEvents() === 2, 'failed shutdown must reopen capture');
...
assertTrue($shutdownResponse->batches === 2, 'recovered shutdown must preserve the failed batch boundary');
assertTrue(
