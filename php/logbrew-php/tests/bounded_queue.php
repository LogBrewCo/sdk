...
assertTrue($contextClient->pendingEvents() === 2, 'count pressure must preserve the existing queue');
assertTrue($contextClient->pendingEventBytes() > 0, 'queued event bytes must be observable');
assertTrue($contextClient->pendingEventBytes() <= 1_048_576, 'queued event bytes must stay bounded');
assertTrue($contextClient->droppedEvents() === 1, 'count pressure must increment the drop total');
assertTrue(count($dropNotices) === 1, 'count pressure must publish one local drop notice');

$notice = $dropNotices[0];
assertTrue($notice->eventId === 'evt_log_dropped', 'drop notice must identify the rejected event');
assertTrue($notice->eventType === 'log', 'drop notice must identify the rejected event type');
assertTrue($notice->reason === 'queue_overflow', 'count pressure must use a stable reason');
assertTrue($notice->droppedEvents === 1, 'drop notice must include the cumulative drop total');
assertTrue($notice->pendingEvents === 2, 'drop notice must include the retained event count');
assertTrue($notice->pendingEventBytes === $contextClient->pendingEventBytes(), 'drop notice must include retained event bytes');
assertTrue(
...
assertTrue(!str_contains($contextClient->previewJson(), 'queue pressure'), 'rejected content must stay out of the queued payload');
...
assertTrue($oversizedClient->pendingEvents() === 0, 'an oversized event must not enter the queue');
assertTrue($oversizedClient->pendingEventBytes() === 0, 'an oversized event must not consume queue bytes');
assertTrue($oversizedClient->droppedEvents() === 1, 'an oversized event must increment the drop total');
assertTrue(count($oversizedNotices) === 1, 'an oversized event must publish one local notice');
assertTrue($oversizedNotices[0]->reason === 'event_too_large', 'an oversized event must use a stable reason');
assertTrue(
...
assertTrue($aggregateByteClient->pendingEvents() === 1, 'aggregate byte pressure must retain the first event');
assertTrue($aggregateByteClient->pendingEventBytes() === $singleEventBytes, 'aggregate byte pressure must retain exact byte accounting');
assertTrue($aggregateByteClient->droppedEvents() === 1, 'aggregate byte pressure must increment the drop total');
assertTrue($aggregateByteReason === 'queue_overflow', 'aggregate byte pressure must use the queue overflow reason');
...
assertTrue($fullQueueClient->pendingEvents() === 1, 'a full queue must retain its existing event');
assertTrue($fullQueueClient->droppedEvents() === 1, 'a full queue must reject without serializing new content');
assertTrue($fullQueueReason === 'queue_overflow', 'a full queue must report count pressure first');
...
assertTrue($callbackFailureClient->pendingEvents() === 1, 'drop callback failures must not change queue state');
assertTrue($callbackFailureClient->droppedEvents() === 1, 'drop callback failures must not hide local loss');
...
assertTrue($failedFlushClient->pendingEvents() === 1, 'failed flush must preserve queued events');
assertTrue($failedFlushClient->pendingEventBytes() === $failedFlushBytes, 'failed flush must preserve queued byte accounting');
...
assertTrue($retryResponse->statusCode === 202, 'shutdown retry must eventually accept the batch');
assertTrue($retryResponse->attempts === 2, 'shutdown retry must report both attempts');
assertTrue(count($retryTransport->sentBodies) === 2, 'shutdown retry must send twice');
assertTrue($retryTransport->sentBodies[0] === $retryTransport->sentBodies[1], 'shutdown retry body must be byte-identical');
assertTrue($retryClient->pendingEvents() === 0, 'successful shutdown must clear queued events');
assertTrue($retryClient->pendingEventBytes() === 0, 'successful shutdown must clear queued byte accounting');
expectSdkError(
...
assertTrue($highLoadClient->pendingEvents() === 1_000, 'default queue must retain exactly 1,000 events');
assertTrue($highLoadClient->droppedEvents() === 500, 'default queue must report exactly 500 dropped events');
assertTrue($defaultDropCount === 500, 'default queue callback must expose the final drop total');
assertTrue($highLoadClient->pendingEventBytes() > 0, 'default queue must expose retained event bytes');
