...
def load_json_object(text, label):
    start = text.find("{")
    require(start >= 0, f"{label} does not contain a JSON object")
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("usage: check_php_http_trace_payload.py stdout.json stderr.json")
    ...
    log_event = event_by_id(events, "php_http_trace_1")
    ...
    require(metadata(log_event).get("psrLevel") == "warning", "PSR level missing")
    require(metadata(log_event).get("context.cartId") == "cart_123", "structured logger metadata missing")
