...
for page in Client Logger RackMiddleware RailsErrorSubscriber RailsRailtie HttpTransport RecordingTransport \
  ...
  test -f "$tmp_dir/rdoc/LogBrew/$page.html"
done
for page in Configuration Runtime RequestMiddleware ErrorReporter; do
  test -f "$tmp_dir/rdoc/LogBrew/Rails/$page.html"
done
...
grep -q 'Bounded Delivery' "$unpacked_dir/README.md"
grep -q 'max_queue_bytes' "$unpacked_dir/README.md"
grep -q 'max_batch_size' "$unpacked_dir/README.md"
grep -q 'max_batch_bytes' "$unpacked_dir/README.md"
grep -q 'response.batches' "$unpacked_dir/README.md"
grep -q 'on_event_dropped' "$unpacked_dir/README.md"
...
grep -q 'Persistent Worker Delivery' "$unpacked_dir/README.md"
...
grep -q '"batches":1' "$tmp_dir/persistent-example.json"
test -f "$tmp_dir/persistent-example-queue/.ack"
