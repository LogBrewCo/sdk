...
def check_payload(payload_path: Path, stderr_path: Path) -> None:
    ...
    _require(
        span["metadata"].get("sampled") is True,
        "first-useful span should expose sampled trace context",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the .NET first-useful telemetry example output.")
    parser.add_argument("payload", type=Path)
    parser.add_argument("stderr", type=Path)
    args = parser.parse_args()
    check_payload(args.payload, args.stderr)
    print("dotnet first-useful telemetry payload ok")
    return 0
