...
def check_payload(payload_path: Path, stderr_path: Path) -> None:
    ...
    for unsafe in FORBIDDEN_PAYLOAD_TEXT:
        _require(unsafe not in payload_text, f"rust first-useful telemetry leaked unsafe value: {unsafe}")
    ...
    _require(
        ...
        f"unexpected rust first-useful event order: {event_types!r}",
    )
    ...
    _require(
        ...
        "rust first-useful product action is missing product_timeline source",
    )
    _require(
        product_metadata.get("routeTemplate") == "/checkout/:cart_id",
        f"unexpected rust product route template: {product_metadata.get('routeTemplate')!r}",
    )

    network_metadata = by_id["evt_action_payment_api"]["attributes"]["metadata"]
    _require(
        network_metadata.get("source") == "network_timeline",
        "rust first-useful network action is missing network_timeline source",
    )
    _require(
        network_metadata.get("routeTemplate") == "/payments/:payment_id",
        f"unexpected rust network route template: {network_metadata.get('routeTemplate')!r}",
    )
    _require(
        network_metadata.get("method") == "POST" and network_metadata.get("statusCode") == 202,
        f"unexpected rust network method/status metadata: {network_metadata!r}",
    )
    ...
    _require(
        ...
        f"unexpected rust first-useful stderr summary: {stderr!r}",
    )
    _require(
        stderr.get("outgoingTraceparent") == EXPECTED_OUTGOING_TRACEPARENT,
        f"unexpected rust outgoing traceparent: {stderr.get('outgoingTraceparent')!r}",
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Rust first-useful telemetry example output.")
    parser.add_argument("payload", type=Path)
    parser.add_argument("stderr", type=Path)
    args = parser.parse_args()
    check_payload(args.payload, args.stderr)
    print("rust first-useful telemetry payload ok")
    return 0
