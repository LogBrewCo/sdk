...
clock_values = iter([10.0, 10.005, 20.0, 20.004, 30.0, 30.002])
...
if len(payload["events"]) != 3:
    raise SystemExit(f"expected three DB-API spans, got {len(payload['events'])}")

update_attributes = payload["events"][0]["attributes"]
select_attributes = payload["events"][1]["attributes"]
error_attributes = payload["events"][2]["attributes"]
