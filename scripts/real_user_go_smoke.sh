...
grep -q '"name": "HTTP GET"' "$tmp_dir/packaged-http-client-trace.stdout.json"
...
if grep -Eq '127\.0\.0\.1|localhost|coupon=summer|receipt|traceparent|spoofed|authorization|accepted|routeTemplate|connectMs|wroteRequestMs|timeToFirstByteMs|responseBodyCompletion' "$tmp_dir/packaged-http-client-trace.stdout.json"; then
