...
final class LogBrewSpringBootCacheManagerPostProcessor implements BeanPostProcessor, Ordered {
    private static final int ORDER = Ordered.LOWEST_PRECEDENCE - 15;
    ...
    LogBrewSpringBootCacheManagerPostProcessor(LogBrewClient client, Environment environment) {
        this(new SingleLogBrewClientProvider(client), environment);
    }

    LogBrewSpringBootCacheManagerPostProcessor(
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (!enabled() || !(bean instanceof CacheManager) || isScopedTarget(beanName)) {
            return bean;
        }
        if (LogBrewSpringCacheTracing.isInstrumentedCacheManager(bean)) {
            return bean;
        }
        ...
        return LogBrewSpringCacheTracing.instrumentCacheManager(
            (CacheManager) bean,
            client,
            LogBrewSpringCacheTracing.CacheConfig.create()
                .system(environment.getProperty("logbrew.cache.system"))
                .eventIdPrefix(environment.getProperty("logbrew.cache.event-id-prefix"))
                .traceWithoutActiveContext(booleanProperty(
                    "logbrew.cache.trace-without-active-context",
                    false
                ))
    ...
    private boolean enabled() {
        return booleanProperty("logbrew.cache.enabled", true);
    }
