...
The request middleware captures one route-template span, one optional `http.server.duration` metric, and one optional exception issue. It keeps `LogBrewTrace.Current` active while downstream handlers run so application-owned logs and telemetry can join the trace. It does not read request or response bodies, capture arbitrary headers, serialize raw `traceparent`, include query strings, or include raw route values; requests without a selected endpoint use the stable `/unmatched` route. The automatic logging provider excludes ASP.NET ambient scopes—which can contain raw paths and connection identifiers—while retaining explicit structured log fields and LogBrew trace correlation. Exception stack traces remain off by default.
