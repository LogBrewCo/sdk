...
namespace LogBrew
{
    ...
    public sealed class IssueExceptionChainEntry
    {
        ...
        private static string RelationshipName(IssueExceptionRelationship value)
        {
            return value switch
            {
                IssueExceptionRelationship.Reported => "reported",
                IssueExceptionRelationship.Cause => "cause",
                IssueExceptionRelationship.Context => "context",
                IssueExceptionRelationship.AggregateMember => "aggregate_member",
                IssueExceptionRelationship.Suppressed => "suppressed",
                _ => throw IssueDiagnostics.Validation("issue exceptionChain relationship is invalid"),
            };
        }
        ...
        private static string MessageStateName(IssueExceptionMessageState value)
        {
            return value switch
            {
                IssueExceptionMessageState.Captured => "captured",
                IssueExceptionMessageState.Truncated => "truncated",
                IssueExceptionMessageState.Redacted => "redacted",
                IssueExceptionMessageState.NotCaptured => "not_captured",
                _ => throw IssueDiagnostics.Validation("issue exceptionChain messageState is invalid"),
            };
        }
        ...
        private static string StackFramesStateName(IssueExceptionStackFramesState value)
        {
            return value switch
            {
                IssueExceptionStackFramesState.Captured => "captured",
                IssueExceptionStackFramesState.Truncated => "truncated",
                IssueExceptionStackFramesState.NotCaptured => "not_captured",
                _ => throw IssueDiagnostics.Validation("issue exceptionChain stackFramesState is invalid"),
            };
        }
    }
                ...
                IssueExceptionRelationship relationship)
            {
                ...
                var mechanism = IssueExceptionMechanism.Create(
                    relationship == IssueExceptionRelationship.AggregateMember
                        ? "dotnet.aggregate_member"
                        : "dotnet.inner_exception",
                    true);
    ...
    /// <summary>Internal structured stack plus explicit omission state.</summary>
