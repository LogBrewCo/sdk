...
import { addBrowserRuntimeContext } from "./runtime-context.js";
import {
...
import { installBrowserLifecycleDelivery } from "./lifecycle-delivery.js";
import { markLifecycleTransport } from "./lifecycle-transport.js";
...
const DEFAULT_SDK_VERSION = "0.1.10";
...
export function captureBrowserError(error, context, options = {}) {
  return captureBrowserIssue(error, context, options, "errorEvent", errorDetails, BROWSER_ERROR_SUMMARY);
}

export function captureUnhandledRejection(rejection, context, options = {}) {
...
export async function captureBrowserNetwork(request, context, options = {}) {
  const eventOptions = eventOptionsWithContext(context, options);
...
export function createBrowserNetworkEvent(request, browserWindow = defaultWindow(), {
...
export function createBrowserErrorEvent(error, browserWindow = defaultWindow(), options = {}) {
  return createBrowserIssueEvent(error, browserWindow, options, errorDetails(error), BROWSER_ERROR_SUMMARY);
}

export function createUnhandledRejectionEvent(rejection, browserWindow = defaultWindow(), options = {}) {
