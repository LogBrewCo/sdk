...
const commands = {
  ...
  "persistent-delivery": "persistent-delivery.mjs",
...
if (command === "--help" || command === "-h") {
  ...
  console.log("  persistent-delivery    Run the opt-in encrypted restart-recovery example");
  ...
  console.log("npm --prefix node_modules/@logbrew/node/examples run persistent-delivery");
  ...
  console.log("node node_modules/@logbrew/node/examples/index.mjs persistent-delivery");
...
if (command === "--list") {
  for (const [name, file] of Object.entries(commands)) {
    console.log(`${name} -> node node_modules/@logbrew/node/examples/index.mjs ${name} (${file})`);
  }
  process.exit(0);
}
