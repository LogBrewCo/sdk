...
export function createReactRouterRouteTemplate(routeMatches, { fallback } = {}) {
  const routeTemplate = routeTemplateFromMatches(routeMatches);
  return routeTemplate ?? stripQueryAndHash(fallback);
}

export function createReactRouterNavigationSpanEvent({
  durationMs,
  id,
...
export function captureReactRouterNavigation(client, input = {}) {
  if (!client) {
...
export function useLogBrewReactRouterNavigation(options = {}) {
  const client = useLogBrew();
