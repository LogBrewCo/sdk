...
function LogBrewProvider({ client, children }) {
  if (!client) {
...
function createReactActionEvent({
  id,
...
function captureReactAction(client, input = {}) {
  if (!client) {
...
function captureReactNetwork(client, input = {}) {
  if (!client) {
...
function createLogBrewReactRouterNavigationObserver({
  useLocation,
  useNavigationType,
  ...defaults
} = {}) {
...
function captureReactError(client, error, options = {}) {
  if (!client) {
...
class LogBrewErrorBoundary extends React.Component {
