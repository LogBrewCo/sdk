...
const { ErrorHandler, InjectionToken, Injector, inject } = require("@angular/core");
const {
  createTraceparent,
  LogBrewClient,
  parseTraceparent,
...
const LOG_BREW_ANGULAR_CONTEXT = new InjectionToken("LogBrew Angular context");

function createLogBrewAngularClient({
...
function createAngularTraceparent({
...
function provideLogBrew(options = {}) {
  const providers = [
    {
...
function createLogBrewAngularContext(client, transport) {
  return {
    client,
    logbrew: client,
    transport,
...
function createAngularViewEvent(name, {
...
function createAngularErrorEvent(error, context = {}, {
  now = () => new Date().toISOString(),
...
async function captureAngularError(error, context, options = {}) {
