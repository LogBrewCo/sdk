...
expect_sdk_error("validation_error", "max_batch_size must be a positive integer") do
  bounded_batching_client(max_batch_size: 0)
end
expect_sdk_error("validation_error", "max_batch_bytes must be a positive integer") do
  bounded_batching_client(max_batch_bytes: 0)
end
expect_sdk_error("validation_error", "max_batch_bytes must fit the SDK envelope") do
  bounded_batching_client(max_batch_bytes: 8)
end
invalid_sdk = "\xB1\x31".dup.force_encoding(Encoding::UTF_8)
expect_sdk_error("validation_error", "sdk_name must be non-empty") do
...
assert(direct_response.batches == 1, "a direct transport response must represent one batch")
empty_response = bounded_batching_client.flush(LogBrew::RecordingTransport.always_accept)
assert(empty_response.batches.zero?, "an empty flush must report zero accepted batches")
tests += 1
...
assert(count_response.status_code == 202, "count split must return the final accepted status")
assert(count_response.attempts == 4, "count split must aggregate transport attempts")
assert(count_response.batches == 3, "count split must report accepted batches")
assert(count_transport.sent_bodies.length == 4, "count split must retry once and send three batches")
assert(count_transport.sent_bodies[0] == count_transport.sent_bodies[1], "retry body must be identical")
assert(!count_transport.sent_bodies[0].include?("\n"), "transport batches must use compact JSON")
assert(
...
assert(count_client.pending_events.zero?, "count splitting must acknowledge every accepted event")
tests += 1
...
assert(byte_response.batches == 2, "exact byte limit must split two events")
assert(
...
assert(oversized_client.pending_events.zero?, "batch-oversized event must not enter the queue")
assert(oversized_notice.reason == "event_too_large", "batch-oversized event must use stable loss reason")
tests += 1
...
assert(admission_bytes.positive?, "admission byte accounting must be observable")
assert(immutable_client.pending_event_bytes.zero?, "accepted immutable event must clear exact byte accounting")
tests += 1
...
assert(identity_preview == expected_identity, "preview SDK identity must be copied at client creation")
assert(identity_wire == expected_identity, "wire SDK identity must match the immutable preview identity")
tests += 1
...
expect_sdk_error("transport_error", "unexpected transport status 500") do
  partial_client.flush(LogBrew::RecordingTransport.new([202, 500]))
end
assert(partial_client.pending_events == 3, "partial success must remove only the accepted prefix")
assert(
...
assert(partial_response.batches == 2, "partial retry must drain two retained batches")
assert(
...
assert(frozen_response.batches == 2, "retry plus later capture must use two accepted batches")
assert(failed_transport.sent_bodies[0] == frozen_retry.sent_bodies[0], "later capture must not change failed body")
assert(
...
assert(reentrant_response.batches == 1, "outer flush must accept its start snapshot")
assert(reentrant_transport.flush_error.code == "flush_error", "reentrant flush must use stable error code")
assert(reentrant_client.pending_events == 1, "transport-time capture must remain queued")
assert(
...
first_transport = BlockingTransport.new
...
second_transport = BlockingTransport.new
...
assert(second_flush.status == "sleep", "second flush must reach a serialized wait")
assert(second_transport.sent_bodies.empty?, "second flush must wait for the active flush")
...
assert(serialized_client.pending_events.zero?, "serialized flushes must drain both snapshots")
tests += 1
...
assert(shutdown_response.batches == 2, "recovered shutdown must preserve the failed boundary")
assert(shutdown_failed.sent_bodies[0] == shutdown_retry.sent_bodies[0], "shutdown retry body must stay frozen")
assert(
