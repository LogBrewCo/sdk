...
expect_sdk_error("validation_error", "max_queue_size must be a positive integer") do
  bounded_queue_client(max_queue_size: 0)
end
expect_sdk_error("validation_error", "max_queue_bytes must be a positive integer") do
  bounded_queue_client(max_queue_bytes: 0)
end
expect_sdk_error("validation_error", "on_event_dropped must respond to call") do
...
expect_sdk_error("validation_error", "event must be JSON serializable") do
...
assert(context_client.pending_events == 2, "count pressure must preserve the existing queue")
assert(context_client.pending_event_bytes.positive?, "queued event bytes must be observable")
assert(context_client.pending_event_bytes <= 1_048_576, "queued event bytes must stay bounded")
assert(context_client.dropped_events == 1, "count pressure must increment the drop total")
assert(drop_notices.length == 1, "count pressure must publish one local drop notice")

notice = drop_notices.fetch(0)
assert(notice.event_id == "evt_log_dropped", "drop notice must identify the rejected event")
assert(notice.event_id.frozen?, "drop notice identifiers must be immutable")
assert(notice.event_type == "log", "drop notice must identify the rejected event type")
assert(notice.reason == "queue_overflow", "count pressure must use a stable reason")
assert(notice.dropped_events == 1, "drop notice must include cumulative loss")
assert(notice.pending_events == 2, "drop notice must include retained event count")
assert(
  notice.pending_event_bytes == context_client.pending_event_bytes,
  "drop notice must include retained event bytes"
)
assert(notice.frozen?, "drop notices must be immutable")
assert(
...
assert(context_events.map { |event| event.fetch("type") } == %w[release environment], "drop-new must retain leading context")
assert(!context_client.preview_json.include?("queue pressure"), "rejected content must stay out of the queue")
tests += 1
...
assert(oversized_client.pending_events.zero?, "an oversized event must not enter the queue")
assert(oversized_client.pending_event_bytes.zero?, "an oversized event must not consume queue bytes")
assert(oversized_client.dropped_events == 1, "an oversized event must increment loss")
assert(oversized_notices.fetch(0).reason == "event_too_large", "oversized events need a stable reason")
assert(
...
assert(aggregate_client.pending_events == 1, "aggregate byte pressure must retain the first event")
assert(aggregate_client.pending_event_bytes == single_event_bytes, "aggregate byte accounting must stay exact")
assert(aggregate_client.dropped_events == 1, "aggregate byte pressure must increment loss")
assert(aggregate_reason == "queue_overflow", "aggregate byte pressure must use queue_overflow")
tests += 1
...
assert(full_queue_client.pending_events == 1, "a full queue must retain its existing event")
assert(full_queue_client.dropped_events == 1, "a full queue must drop before serialization")
assert(full_queue_reason == "queue_overflow", "count pressure must take precedence")
tests += 1
...
assert(callback_failure_client.pending_events == 1, "callback failures must not change queue state")
assert(callback_failure_client.dropped_events == 1, "callback failures must not hide loss")
tests += 1
...
assert(reentrant_calls == 1, "drop callbacks must not recursively invoke themselves")
assert(reentrant_client.dropped_events == 2, "reentrant capture attempts must remain visible")
tests += 1
...
assert(failed_flush_client.pending_events == 1, "failed flush must preserve events")
assert(failed_flush_client.pending_event_bytes == failed_flush_bytes, "failed flush must preserve bytes")
tests += 1
...
assert(retry_response.status_code == 202, "shutdown retry must accept the batch")
assert(retry_response.attempts == 2, "shutdown retry must report both attempts")
assert(retry_transport.sent_bodies.length == 2, "shutdown retry must send twice")
assert(retry_transport.sent_bodies[0] == retry_transport.sent_bodies[1], "retry bodies must be byte-identical")
assert(retry_client.pending_events.zero?, "shutdown must clear accepted events")
assert(retry_client.pending_event_bytes.zero?, "shutdown must clear accepted bytes")
...
blocking_flush_transport = BlockingTransport.new
...
assert(remaining_events.length == 1, "successful flush must retain concurrently captured events")
assert(remaining_events.fetch(0).fetch("id") == "evt_during_flush", "flush must acknowledge only its snapshot")
second_flush_transport = LogBrew::RecordingTransport.always_accept
concurrent_flush_client.flush(second_flush_transport)
assert(second_flush_transport.last_body.include?("evt_during_flush"), "next flush must send the retained event")
tests += 1
...
blocking_shutdown_transport = BlockingTransport.new
...
expect_sdk_error("shutdown_error", "client is shutting down") do
...
assert(shutdown_client.pending_events.zero?, "successful shutdown must leave no stranded events")
tests += 1
...
blocking_failed_shutdown = BlockingTransport.new(status: 400)
...
assert(failed_shutdown_result.is_a?(LogBrew::SdkError), "failed shutdown must return its transport error")
assert(failed_shutdown_result.code == "transport_error", "failed shutdown must preserve transport classification")
failed_shutdown_client.log("evt_after_failed_shutdown", "2026-07-12T10:00:22Z", message: "accepted", level: "info")
assert(failed_shutdown_client.pending_events == 2, "failed shutdown must reopen capture and preserve its queue")
tests += 1
...
workers.each(&:join)
assert(high_load_client.pending_events == 1_000, "concurrent default queue must retain exactly 1,000 events")
assert(high_load_client.dropped_events == 9_000, "concurrent default queue must report exactly 9,000 drops")
assert(drop_callback_count == 9_000, "concurrent callback count must match local loss")
assert(high_load_client.pending_event_bytes.positive?, "concurrent queue must expose retained bytes")
assert(high_load_client.pending_event_bytes <= 4_194_304, "concurrent queue must honor its byte bound")
tests += 1
